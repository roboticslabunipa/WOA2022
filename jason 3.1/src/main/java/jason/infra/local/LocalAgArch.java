...
public class LocalAgArch extends AgArch implements Runnable, Serializable {
    ...
    // ---------- NEW ACT -------------------
    protected ActionExec actExtended() {
        TransitionSystem ts = getTS();

        ActionExec actionPerformed = null;
        int i = 0;
        int ca = cyclesAct;
        if (ca != 1) { // not the default value, limit the value to the number of intentions
            ca = Math.min(cyclesAct, ts.getC().getNbRunningIntentions());
    ...
    // ---------- NEW STEP -------------------
    protected void newStepReasoning(ActionExec actionPerformed){
        if (actionPerformed != null){
            TransitionSystem ts = getTS();
            ts.newStep(actionPerformed);
        }
    }
    // ---------- END -------------------
    ...
    // ---------- NEW RC -------------------
    protected void extendedReasoningCycle() {
        getFirstAgArch().reasoningCycleStarting();

        sense();
        deliberate();
        ActionExec actionPerformed = actExtended();
        newStepReasoning(actionPerformed);

        getFirstAgArch().reasoningCycleFinished();
    }
    // ---------- END -------------------

    // To restore the original loop change the function extendedReasoningCycle() with the default method reasoningCycle()
    public void run() {
        TransitionSystem ts = getTS();
        while (running) {
            if (ts.getSettings().isSync()) {
                waitSyncSignal();
                //extendedReasoningCycle();
                reasoningCycle();
    ...
    public void checkMail() {
        ...
        while (im != null) {
            // (NEW) add simple logic to set priority
            if(im.getSender().equals(this.getAgName()))
                im.setPriority(1);
            C.addMsg(im);
    ...
    public Queue<Message> getMBox() {
        return mbox;
    }

    // (NEW) function to retrieve only last msg received
