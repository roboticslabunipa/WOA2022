...
public class send extends DefaultInternalAction {
    ...
    private boolean lastSendWasSynAskHow = false;
    ...
    public Object execute(final TransitionSystem ts, Unifier un, Term[] args) throws Exception {
        ...
        // (NEW) sync ask how message
        // lastSendWasSynAskHow = m.isAskHow() && args.length > 3;
        // if (lastSendWasSynAskHow) {
        //     m.setSyncAskHowMsgId();
        //     ts.getC().addPendingIntention(m.getMsgId(), ASSyntax.createAtom("waiting_ask_how"), ts.getC().getSelectedIntention(), false);
        // }
        ...
        //if ((lastSendWasSynAsk && args.length == 5) || (lastSendWasSynAskHow && args.length == 4)) {
            // (NEW) set the correct idx
            //int idx = (args.length == 5) ? 4 : 3;
            ...
            if (tto.isNumeric()) {
                ...
                throw new JasonException("The last parameter of send must be a number (timeout) and not '"+tto+"'!");
            }
        }
