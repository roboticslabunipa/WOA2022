...
public class Agent implements Serializable, ToDOM {
    ...
    public static Agent create(AgArch arch, String agClass, ClassParameters bbPars, String asSrc, Settings stts) throws Exception {
        //try {
            ...
            // if(agClass.equals("ISAgent"))
            //     ag = new ISAgent();
            // else
            //     ag = (Agent) Class.forName(agClass).getConstructor().newInstance();
    ...
    */
    ...
    //     gestendo il caso in cui solo alcuni non sono raggiungibili */
    // public boolean requestForPlan(Trigger te) throws Exception {
    //     try {
    //         //logger.info(getClass().getName()+" -> Request plan for event "+te.getLiteral().clearAnnots().toString());
    //         Message m = new Message("ask-how", ts.getAgArch().getAgName(), null, te.getTriggerAsLiteral());
