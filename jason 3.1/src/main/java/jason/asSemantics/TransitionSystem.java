...
public class TransitionSystem implements Serializable {
    ...
    // (NEW) state WaitRelPl
    public enum State { StartRC, SelEv, RelPl, WaitRelPl, ApplPl, SelAppl, FindOp, AddIM, ProcAct, SelInt, ExecInt, ClrInt }
    ...
    private void applySemanticRuleDeliberate() throws JasonException, Exception {
    ...
    */
    ...
    private void applyRelPl() throws JasonException, Exception {
        if (C.SE.getOption() == null) { // no option yet
            ...
            // ----- (NEW) state for retrieve plan during execution ---
            else if(ag.getClass().getName().contains("ISAgent") && C.RP == null && !C.SE.trigger.isUpdate())
                // try to retrieve relevant plan for the specified event from other agents: add a new state 
                stepDeliberate = State.WaitRelPl;
            // -------------- END ----------------
            else
    ...
    // ----- (NEW) -----
    private void applyWaitRelPl() throws Exception{
        if(((ISAgent)ag).requestForPlan(C.SE.trigger)){
            long msEnd = System.currentTimeMillis() + 5000;
            //wait 5 second for a message
            while (System.currentTimeMillis() <= msEnd) {
                Message m = ag.selectMessage(getAgArch().getMBox());

                if(m == null){
                    logger.info("Wait messagges..");
                }
                //else if(m.isTellHow() && ((Plan) ((List) m.getPropCont()).get(0)).getTrigger().equals(C.SE.trigger)){
                else if(m.isTellHow()){
                    List<Plan> pl = (List<Plan>) m.getPropCont();
                    Plan p = pl.get(0);
                    if(p != null && p.getTrigger().equals(C.SE.trigger)){
                        ag.getPL().addAll(pl);
                        // C.RP = relevantPlans(C.SE.trigger, C.SE);
                        // stepDeliberate = State.ApplPl;
                        stepDeliberate = State.RelPl;
        ...
        applyRelApplPlRule2("wait_relevant");
    }
    // ---- END -----
    ...
    private boolean generateGoalDeletionFromEvent(List<Term> failAnnots, Term reason) throws JasonException {
        ...
        if (tevent.isAddition() && tevent.isGoal()) {
            ...
            if (failEvent != null) {
                ...
                //createGoalDeletionFromEvent(reason, ev);
            }
    ...
    /* ------ NEW function to create an achievement-goal deletion (ext-event discard!!!)------
    private void createGoalDeletionFromEvent(Term reason, Event ev){
        Event delEv = new Event(new Trigger(TEOperator.del, ev.getTrigger().getType(), ev.getTrigger().getLiteral()), ev.intention);
        setFailureAnnot(delEv, JasonException.createMinimalErrorAnnot(reason));
        logger.info(delEv.toString());
        updateEvents(delEv);
    }
    // --------------------------------------------------------  */
    ...
    // -------- NEW --------
    private static void setFailureAnnot(Event failEvent, Term error){
        if(error == null)
            error = JasonException.createMinimalErrorAnnot(JasonException.UNKNOW_ERROR);

        Literal eventLiteral = failEvent.getTrigger().getLiteral().forceFullLiteralImpl();
        eventLiteral.addAnnot(error);
    }

    private static void setFailureAnnots(Event failEvent, List<Term> errors){
        if(errors == null)
            errors = JasonException.createBasicErrorAnnots( JasonException.UNKNOW_ERROR, "");

        Literal eventLiteral = failEvent.getTrigger().getLiteral().forceFullLiteralImpl();
        eventLiteral.addAnnots(errors);
    }
    // ------------------
    ...
    */
    ...
    // --- (NEW) Method modify to extend the reasoning cycle **TEST** ---
    public ActionExec act() {
        try {
            C.resetAct();
    ...
    // ---------------------- NEW ------------------------------- 
    public void newStep(ActionExec a) {
        try {
            Intention currInt = a.getIntention();
            // NOT WORK
            boolean isUpdated = ag.buf(getAgArch().perceive()) != 0 ? true : false;
            //logger.info(isUpdated ? "Y" : "N");
        } catch (Exception e) {
            logger.log(Level.SEVERE, "*** ERROR in the transition system (newStep). "+C+"\nCreating a new C!", e);
            C.create();
        }
    }
    // ------------------------ END -----------------------------
