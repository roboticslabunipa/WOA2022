...
public class Circumstance implements Serializable, ToDOM {
    ...
    /* Add priority to different event:
        - use PriorityBlockingQueue class instead of ConcurrentLinkedQueue class with a Comparator
          (is not the same because the first is blocking, the second no);
        - add a new Queue in which store only high priority Event 
          (this means we have only event with low/high priority);
        - manually search the event desiderated    
    */
